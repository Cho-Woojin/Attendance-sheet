...
from models import reset_logs_with_timestamp,load_holidays, save_holidays, is_valid_day_and_time, has_record, calculate_weekly_data
...
def write_to_csv(student_id, action_type):
    """
    CSV에 출근/퇴근 기록을 추가합니다.
    """
    today = datetime.now().strftime("%Y-%m-%d")
    now = datetime.now()
    rounded_time = round_time_to_decimal(now)
    with open(LOG_FILE, mode="a", newline="", encoding="utf-8") as file:
        writer = csv.writer(file)
        writer.writerow([student_id, today, rounded_time, action_type])
...
def init_routes(app):
    ...
    def home():
        ...
        return render_template("home.html", date=formatted_date, time=current_time, message=None)

    # 출근/퇴근 기록
    @app.route("/record", methods=["POST"])
    def record():
        student_data = load_student_data()  # 학생 정보 로드
        student_id = request.form.get("student_id", "").strip()  # 학번 입력, 누락 시 기본값 ""
        action = request.form.get("action", "").strip()  # 출근 또는 퇴근 액션 값, 누락 시 기본값 ""
        
        # 학번 또는 액션 값 누락 시
        if not student_id or not action:
            return render_home("학번과 액션 값이 필요합니다. 입력 후 다시 시도해주세요.")
        
        current_time = datetime.now().strftime("%p %I:%M").replace("AM", "오전").replace("PM", "오후")
        ...
        if not is_valid_day_and_time():
            return render_home(f"{student_name}님, 출퇴근 가능 시간이 아닙니다. (평일 08:00 ~ 22:00)")
        ...
        if action == "check_in":
            ...
            return render_home(f"{student_name}님, {current_time} 출근 기록이 추가되었습니다.")

        # 퇴근 처리
        elif action == "check_out":
            if not has_record(student_id, "출근"):
                return render_home(f"{student_name}님, 출근 기록이 없습니다. 먼저 출근을 기록해주세요.")
            if has_record(student_id, "퇴근"):
                return render_home(f"{student_name}님, 이미 퇴근 기록이 존재합니다.")
            write_to_csv(student_id, "퇴근")
            return render_home(f"{student_name}님, {current_time} 퇴근 기록이 추가되었습니다.")
    ...
    def weekly():
        ...
        if request.method == "POST":
            ...
            week_start = week_start - timedelta(days=week_start.weekday())  # 해당 주의 월요일로 설정
        else:
            # 기본적으로 현재 주차로 설정
        ...
        if week_start.date() > today:
            week_data = {student_data[student_id]: {} for student_id in student_data.keys()}
            ...
            for student_id, student_name in student_data.items():
                student_days = week_data.get(student_id, {
                    day: {"출근": None, "퇴근": None, "근무시간": "0.0"} for day in ["월요일", "화요일", "수요일", "목요일", "금요일"]
                })
            ...
            for student_name, days in week_data.items():
                total_time = sum(float(day["근무시간"]) for day in days.values() if day["근무시간"] != "0.0")
    ...
    def manage_holidays():
        ...
        if request.method == "POST":
            ...
            if action == "add":
                ...
                selected_dates = request.form.getlist("selected_dates")  # 다중 선택된 날짜
    ...
    def render_home(message):
        ...
        return render_template("home.html", date=get_current_date(), time=get_current_time(), message=message)
